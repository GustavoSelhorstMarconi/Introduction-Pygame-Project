...
class Boss(pygame.sprite.Sprite):
  def __init__(self):
    ...
    self.image = pygame.transform.scale(mario, (131, 118))
    self.image = pygame.transform.flip(self.image, True, False)
    self.rect = self.image.get_rect(bottomright = (750, 320))
    self.direction  = pygame.math.Vector2(-1, 0)
    self.gravity = 0
  
  def movimentacao(self):
    for event in events:
      if event.type == use_fireball and boss.sprites():
        self.gravity = -30
...
class Fireball(pygame.sprite.Sprite):
  def __init__(self):
    ...
    self.rect = self.image.get_rect(topright = (650, 250))
...
def collision_sprite():
  if pygame.sprite.spritecollide(player.sprite, obstacle_group, False):
    ...
    boss.empty()
    return False
  elif pygame.sprite.spritecollide(player.sprite, boss, False):
    boss.empty()
    obstacle_group.empty()
    return False
  elif pygame.sprite.spritecollide(player.sprite, fireball, False):
    boss.empty()
    obstacle_group.empty()
    fireball.empty()
    return False
...
use_fireball = pygame.USEREVENT + 4
pygame.time.set_timer(use_fireball, 2000)
...
while True:
  ...
  for event in events:
    ...
    if game_active:
      ...
      if score % 10 == 0 and score != 0 and boss.has(boss) == False:
        boss.add(Boss())
